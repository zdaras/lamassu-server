...
// TODO: Check for isLightning, if not, use c.walletBalance instead
...
function newAddress (account, info) {
  return checkCryptoCode(info.cryptoCode)
    ...
    .then(c => {
      if (info.isLightning) {
        const params = {
          memo: 'Lamassu cryptomat deposit',
          value: info.cryptoAtoms.toNumber()
        }

        return c.addInvoice(params)
...
function getStatus (account, toAddress, requested, cryptoCode) {
  return checkCryptoCode(cryptoCode)
    .then(() => {
      ...
      // TODO: Delegate to bitcoind wallet?
      if (!isLightning) return {status: 'notSeen'}
